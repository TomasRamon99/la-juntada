...
function seed(){
  ...
  return {people:ppl, items, rounding:0, planType:"asado", planSplit:true, planDrink:true, planDessert:false, planH:4, planM:3, planN:7, rules:DEFAULT_RULES.slice(), settled:[], nombre:""};
}
function normalize(s){
  ...
  if(typeof s.nombre !== "string") s.nombre = "";
...
function renderAll(){ renderPeople(); renderItems(); renderResult(); renderPlan(); renderRules(); updateNameSuggestions(); renderNombre(); }
function renderNombre(){
  const el=$("#juntada-nombre"); if(!el) return;
...
// ---- Reset ----
$("#btn-reset").onclick=()=>{ if(confirm("¿Vaciar todo y empezar una juntada nueva?")){ state={people:[],items:[],rounding:0,settled:[],nombre:"",planType:state.planType,planSplit:state.planSplit,planDrink:state.planDrink,planDessert:state.planDessert,planH:state.planH,planM:state.planM,planN:state.planN,rules:state.rules}; history.replaceState(null,"",location.pathname); document.querySelectorAll("#round-seg button").forEach(b=>b.classList.toggle("on",b.dataset.r==="0")); save(); renderAll(); } };

// ---- Bienvenida ----
$("#welcome-start").onclick=()=>{
  // Resetear campo y abrir dialog
  const inp=$("#nueva-nombre"); if(inp) inp.value="";
  const fd=$("#nueva-fecha"); if(fd) fd.value=new Date().toISOString().slice(0,10);
  $("#dlg-nueva").showModal();
};
$("#nueva-cancel").onclick=()=>$("#dlg-nueva").close();
$("#nueva-ok").onclick=()=>{
  const nombre=($("#nueva-nombre").value||"").trim();
  const fecha=$("#nueva-fecha").value||"";
  // Resetear state desde cero
  state={people:[],items:[],rounding:0,settled:[],
    nombre: nombre || (fecha ? "Juntada "+fecha : "Juntada"),
    planType:state.planType,planSplit:state.planSplit,planDrink:state.planDrink,
    planDessert:state.planDessert,planH:state.planH,planM:state.planM,planN:state.planN,
    rules:state.rules};
  save(); renderAll();
  $("#dlg-nueva").close();
